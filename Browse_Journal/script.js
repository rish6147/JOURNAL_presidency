...
var userAgent = navigator.userAgent.toLowerCase(),
  ...
  fullDetailsBlock = document.getElementsByClassName('block-with-full-details'),
...
// Mobile Screen Menu Click Toggle Menu Section
var windowWidht = $(window).width();
if( windowWidht < 999){
  $(document).on('click', 'ul.rd-navbar-nav li a', function(){
    $('.rd-navbar-toggle.toggle-original').trigger('click');
  })
}
...
$document.ready(function () {
  ...
  if (plugins.pageLoader.length > 0) {
    $window.on("load", function () {
      var i,
        $bgImage = $('.section-bg-image .section-bg-image-element .section-bg-image-element-inner');
      for ( i = 0; i < $bgImage.length; i++) {
        var o = $($bgImage[i]);
        o.css({'transition' : 'transform 20000ms linear', 'transform': 'scale(1.2, 1.2)'});
      }
